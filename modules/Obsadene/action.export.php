...
if (!$this->CheckAccess('Manage Obsadene')) {
...
if (MCFTools::IsModuleActive('CMSExcel'))
{
	$params = array(
		'creator' => 'M&C Factory',
		'title' => 'Obsadene export',
		'subject' => 'Obsadene export',
		);
	
	$stack = cms_utils::get_module('CMSExcel')->buildExcel($datas,$params);
	
	$link = cms_utils::get_module('CMSExcel')->getExcelLink($stack,array('filename' => 'Obsadene_export_'.date('Y-m-d_H-i-s').'.xls'));

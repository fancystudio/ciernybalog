...
class Obsadene extends CMSModule {
  ...
  public function GetName() {               return 'Obsadene';  }
  public function GetObjectName() {         return 'ObsadeneObject';  }
  public function GetFriendlyName() {       return 'Obsadene';  }
  public function GetVersion() {            return '4';  }
  ...
  public  function GetDependencies() {      return array('MCFactory' => '3.4.98');  }
  public  function CheckAccess($permission = 'Manage Obsadene') {    return $this->CheckPermission($permission);  }
  ...
  public function Install() {
    ...
    $sqlarray = $dict->CreateTableSQL(cms_db_prefix().'module_obsadene', implode(',',$fields));
    $dict->ExecuteSQLArray($sqlarray);
    $db->CreateSequence(cms_db_prefix().'module_obsadene_seq');
  ...
  public function Upgrade($oldversion, $newversion)  {
    ...
    switch(true) {
      ...
      case version_compare($oldversion, '2.9.16', '<'):
          $sqlarray = $dict->AddColumnSQL(cms_db_prefix() . 'module_obsadene', 'user_id I');
          $dict->ExecuteSQLArray($sqlarray);
      case version_compare($oldversion, '2.12.1', '<'):
          $sqlarray = $dict->AddColumnSQL(cms_db_prefix() . 'module_obsadene', 'mcfi_created_timestamp I');
          $dict->ExecuteSQLArray($sqlarray);
          $sqlarray = $dict->AddColumnSQL(cms_db_prefix() . 'module_obsadene', 'mcfi_updated_timestamp I');
          $dict->ExecuteSQLArray($sqlarray);
      case version_compare($oldversion, '3.1.8', '<'):
          $sqlarray = $dict->AddColumnSQL(cms_db_prefix() . 'module_obsadene', 'send_update_immediately I');
          $dict->ExecuteSQLArray($sqlarray);
  ...
  public function Uninstall() {
    $db = cms_utils::get_db();
    $dict = NewDataDictionary($db);
    $sql = $dict->DropTableSQL(cms_db_prefix().'module_obsadene');
    $dict->ExecuteSQLArray($sql);
    $db->DropSequence(cms_db_prefix().'module_obsadene_seq');
  ...
  function InitializeGlobal() {  
    ...
    // $this->RegisterRoute('/obsadene\/(?P<item_id>[0-9]+)(\/.*?)?$/', array('action' => 'detail', 'returnid' => cmsms()->GetContentOperations()->GetDefaultPageID()));
    $this->RegisterRoute('/obsadene\/(?P<item_id>[0-9]+)\/(?P<returnid>[0-9]+)(\/.*?)?$/', array('action' => 'detail'));
    $this->RegisterRoute('/obsadene\/rss\/(?P<maction>[a-zA-Z0-9_-]+)(\/.*?)?$/', array('action' => 'rss', 'showtemplate' => 'false','returnid' => cmsms()->GetContentOperations()->GetDefaultPageID()));
    $this->RegisterRoute('/obsadene\/api\/(?P<command>[a-zA-Z0-9_-]+)(\/.*?)?$/', array('action' => 'api', 'showtemplate' => 'false','returnid' => cmsms()->GetContentOperations()->GetDefaultPageID()));
    
    $this->RegisterRoute('/obsadene\/download\/(?P<item_id>[0-9]+)\/(?P<field>[a-zA-Z0-9_-]+)(\/.*?)?$/', 
      array(
      'action' => 'download', 
      'showtemplate' => 'false',
  ...
  function createLink($id, $action, $returnid='', $contents='', $params=array(), $warn_message='', $onlyhref=false, $inline=false, $addttext='', $targetcontentonly=false, $prettyurl='',  $withslash = false) {
    ...
    if (empty($prettyurl)) {
      if ($action == 'detail') {
        ...
        $prettyurl = 'obsadene/' . $item_id . '/' . $returnid;
  ...
  function deindex($item) {
    $search = $this->GetModuleInstance('Search');
    if ($search) {
      $search->DeleteWords('Obsadene', $item->getId(), 'obsadene_item');
    }
  }

  function SearchResult($returnid, $id, $attr = '') {
    $result = array();
    if ($attr == 'obsadene_item') {
      $c = new MCFCriteria();
      $c->add('id', $id);
      $c->add('published', 1);
...
// TODO:     $c->addAscendingOrderByColumn('mcfi_updated_timestamp');
  ...
  {
    $item = ObsadeneObject::retrieveByPk($item_id);
  ...
  public static function ExportDatas()  {
    ...
    foreach($items as $item)
    {
      $datas[] = array(
        ...
        'title' => $item->getTitle(),
                            'Dtum' => $item->getDtum(),
                                      'Je Obsadene' => $item->getJeObsadene(),
  ...
  {
    // DEPRECATED
    ObsadeneObject::buildFiltersCriteria($c, $filters);
  }
  ...
  public static function getTitleLabel()
  {
	...
	{
		return ObsadeneObject::doSelect($c);
	}
}

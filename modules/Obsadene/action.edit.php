...
if (isset($params['item_id']) && !empty($params['item_id'])) {
  ...
  $item = new ObsadeneObject();
  $item->setParentItem(isset($params['parent_item']) ? $params['parent_item'] : 0);
  
}

// - - - - nasatavenie zaznamu podla posledneho pridaneho
$filds = array();
$filds = ObsadeneObject::getActualDate();
if($filds['je_obsadene'] != null && isset($filds['je_obsadene'])){
	$item->setJeObsadene($filds['je_obsadene']);
}
if($filds['datum'] != null && isset($filds['datum'])){
	$item->setDtum($filds['datum']);
}
//------------------------------------------------------
...
$form = new CMSForm('Obsadene', $id,'edit',$returnid);
...
ObsadeneViews::createForm($this,$form,$item);
...
if (is_object($modulextender) && $modulextender->isXtendedModule($this->getName())) {  
  $this->smarty->assign('xtended_form', $modulextender->XtendModuleForm('Obsadene', $item->getId(), $id));
} else {
...
if ($form->isPosted()) {
  ...
  if ($form->NoError())
  {
    ...
    $item->uploadFiles($id);
    //$item->save(); //klasicke ulozenie do tabulky cms_module_obsadene
    $item->saveDate(); //klasicke ulozenie do tabulky cms_module_obsadene_date
...
$this->smarty->assign('defaultDate', "'".date("d.m.Y", strtotime($item->dtum))."'");
$this->smarty->assign('dates', "['".implode("','",$item->getDates())."']");

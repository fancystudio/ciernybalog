...
public static function adminItems($module,$id,$returnid,$view,$items,$parent_id=null)
{
	...
	// Preview code
	if (!isset($_SESSION['Obsadene']['preview_key']))
	{
		$_SESSION['Obsadene']['preview_key'] = base64_encode(time());
	}
	...
	foreach ($items as $item) {
		...
		$rows[$item->getId()] = array(
		  ...
		  'parent_id' => $item->getParentId(),
			...
			'previewlink' => $module->CreateLink($id, 'detail', $module->getPreference('default_page'), cmsms()->get_variable('admintheme')->DisplayImage('icons/system/view.gif', $module->lang('preview_s',$item->getTitle()), '', '', 'systemicon'), array('preview_key' => $_SESSION['Obsadene']['preview_key'], 'item_id' => $item->getId()), '', false,false,' target="_new"', true)
  ...
  if(class_exists('MX_Relation'))
  {
	...
	if (is_null($parent_id))
	{
		...
		ObsadeneObject::buildFiltersWidgets($form, $currentModuleFilters);		
...
public static function createForm($module,&$form,&$item,$params=array()){
    ...
    if(!isset($params['frontend'])) {        	
        $form->getFieldset('default---default')->setWidget('dtum', 'date', array(
	  		'label' => 'Dátum', 
	  		'object' => &$item, 
			'european_date' => true, 
	  		'set_method' => 'setDtum',		
	  		'default_value' => isset($params['dtum'])?$params['dtum']:array(date('Y'),date('m'),date('d')), 
	  		'start_year' => 2013  		  		  	
  		));
   	}	  
	if(!isset($params['frontend'])) {          
      	$form->getFieldset('default---default')->setWidget('je_obsadene', 'select', array(
	      'values' => array('1' => 'My checkbox text'),
	      'label' => 'je obsadene',
	      'object' => &$item,
	      'get_method' => 'getJeObsadene', 
	      'multiple' => true,
	      'expanded' => true
        ));
	}	

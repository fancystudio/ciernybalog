...
class ObsadeneObjectBase extends MCFModuleObject {

  protected $childrens;
  protected $level;

  const MODULE_NAME = 'Obsadene';
  const MODULE_OBJECT_NAME = 'ObsadeneObject';

  const DB_NAME = 'module_obsadene';
  const DB_ITEM = 'obsadene';
  const UPLOADS_RELATIVE_URL = '/uploads/Modules/Obsadene';

  protected static $fields = array('title',  'parent_id',  'parent_item',  'order_by', 'published', 'send_update_immediately', 'dtum',   'je_obsadene',    'full_text_search'  );
  ...
  public function getParentItemObject() {
    return ObsadeneObject::doSelectOne(array('where' => array('id' => $this->parent_item)));
  }
  ...
  /**
    * @param bool $parse
    */
  public function getDtum($parse = false) {
    if ($parse) {
      return cms_module_ProcessTemplateFromData(new stdClass(), $this->dtum);
    }
    return $this->dtum;
  }

  public function setDtum($value) {
        if (is_array($value)) {
  ...
  /**
    * @param bool $parse
    */
  public function getJeObsadene($parse = false) {
    if ($parse) {
      return cms_module_ProcessTemplateFromData(new stdClass(), $this->je_obsadene);
    }
    return $this->je_obsadene;
  }

  public function setJeObsadene($value) {
        $this->__set('je_obsadene', $value);
  }
...
//    unset($array['full_text_search']);
  ...
  public static function getActualDate(){
  	$return = array();
  	$db = cms_utils::get_db();
    $query = 'SELECT je_obsadene,dtum FROM cms_module_obsadene_date order by updated_at DESC LIMIT 1';
	$result = $db->execute($query, array());
	if(!$result) return false;
    $row = $result->FetchRow();
    $return['datum'] = $row['dtum'];
    $return['je_obsadene'] = $row['je_obsadene'];
    return $return;
  }
  ...
  public function populateFromArray(array $params, $prefix = null) {
    ...
    if (isset($params[$prefix.'full_text_search']))
    {
      $this->setFullTextSearch($params[$prefix.'full_text_search']);
        ...
        if (isset($params[$prefix.'dtum'])) {
      $this->setDtum($params[$prefix.'dtum']);
    }
        if (isset($params[$prefix.'je_obsadene'])) {
      $this->setJeObsadene($params[$prefix.'je_obsadene']);
    }
      }
  ...
  public function toArray()
  {
    ...
    $array['published'] = $this->published;
        ...
        $array['dtum'] = $this->getDtum();
        $array['je_obsadene'] = $this->getJeObsadene();
...
public function saveDate($params = array(), $force_save = false) {
	$this->insertDate($params);
    cms_utils::get_module(self::MODULE_NAME)->index($this);
    return true;
  }
  
protected function insertDate($params = array()) {
    $db = cms_utils::get_db();
    $query = 'SELECT 88 FROM cms_module_obsadene_date where dtum = ?';
	$values = array();
    $values[] = $this->dtum;

...
ObsadeneObject::globalFrontendFilters($c, $params);
...
elseif (
	...
	&& 
	isset($_SESSION['modules']['Obsadene']['calendar']['scope'])
	&&
...
{
	 $calendar = new MCFCalendar($_SESSION['modules']['Obsadene']['calendar']['scope']);
   $c->add('start_date', date('Y/m/d', $calendar->end_date), MCFCriteria::LESS_EQUAL);
   $c->add('end_date', date('Y/m/d', $calendar->start_date), MCFCriteria::GREATER_EQUAL);
}
...
if(is_object($modulextender))  {
  if ($modulextender->isXtendedModule('Obsadene')) {
    $mxfilters = array();
...
$items = ObsadeneObject::doSelect($c);
...
{
  $tree = ObsadeneObject::buildTree($items);
  $this->smarty->assign('obsadene_tree', $tree); 
}

if (isset($params['limit'])) {
	$c->setOffset(0);
	$c->setLimit(0);
	$total = ObsadeneObject::doCount($c);
...
if (isset($params['var'])) {
    ...
    $this->smarty->assign('Obsadene', $items);
    $this->smarty->assign('obsadene', $items);
    $paramsobj = new stdClass();
    $paramsobj->params = $params;
    $this->smarty->assign('mcfactory', $paramsobj);
    $this->smarty->assign('obsadene_params', $paramsobj);

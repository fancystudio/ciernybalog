...
if( isset($params['cancel']) ) {
  ...
  if( empty($error) ) {
    ...
    $query = 'SELECT id FROM '.cms_db_prefix().'module_cggooglemaps_icons WHERE name = ? AND id != ?';
  ...
  if( empty($error) ) {
    if( $iconid > 0 ) {
      // do the update
      $query = 'UPDATE '.cms_db_prefix().'module_cggooglemaps_icons
      ...
      // do the insert
      $query = 'INSERT INTO '.cms_db_prefix().'module_cggooglemaps_icons
